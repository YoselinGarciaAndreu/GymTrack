...
export class CardsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.getEjerciciosDeEspalda();

  }
  getEjerciciosDeEspalda(): void {
    this.ejerciciosService.getEjerciciosByTipo('Espalda').subscribe({
      next: (data: Ejercicio[]) => {
  ...
  getEjerciciosDeEspaldaBusqueda(nombre : string): void {
  ...
  getEjerciciosDeTipoBusqueda(nombre : string): void {
    this.ejerciciosService.getEjerciciosByNombreBusquedaByTipo(nombre, "Espalda").subscribe({
      next: (data) => {
        this.cards = data;
      },
      error: (error) => {
